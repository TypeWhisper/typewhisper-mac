...
@testable import InferenceAPIsPlugin

final class InferenceAPIsPluginTests: XCTestCase {
    ...
    func testTranscribeUsesLongTimeoutForLargerAudioUploads() async throws {
        let host = try PluginTestHostServices(
            defaults: ["selectedModel": "openai/whisper-large-v3"],
            secrets: ["api-key": "ia-key"]
        )
        ...
        PluginHTTPClientTestHarness.configure { _ in
            store.makeSession(outcomes: [
                .success(
                    ...
                    Self.httpResponse(
                        url: "https://api.inferenceapis.com/v1/audio/transcriptions",
                        statusCode: 200
                    )
                ),
            ])
    ...
    func testTranscribeRetriesWithWavWhenInferenceAPIsRejectsM4AUpload() async throws {
        let host = try PluginTestHostServices(
            defaults: ["selectedModel": "openai/whisper-large-v3"],
            secrets: ["api-key": "ia-key"]
        )
    ...
    func testProcessSendsReasoningEffortNoneForReasoningModels() async throws {
        let host = try PluginTestHostServices(
            defaults: ["selectedLLMModel": "deepseek-ai/DeepSeek-V4.1-Flash"],
            secrets: ["api-key": "ia-key"]
        )
        let plugin = InferenceAPIsPlugin()
        plugin.activate(host: host)
        ...
        PluginHTTPClientTestHarness.configure { _ in
            store.makeSession(outcomes: [
                .success(
                    Data(#"{"choices":[{"message":{"role":"assistant","content":"Hello, world."}}]}"#.utf8),
                    Self.httpResponse(
        ...
        let result = try await plugin.process(systemPrompt: "Fix grammar", userText: "hello world", model: nil)
        ...
        XCTAssertEqual(json["model"] as? String, "deepseek-ai/DeepSeek-V4.1-Flash")
    }

    func testProcessOmitsReasoningEffortForNonReasoningModel() async throws {
        let host = try PluginTestHostServices(
            defaults: ["selectedLLMModel": "meta-llama/Llama-3.3-70B-Instruct-Turbo"],
            secrets: ["api-key": "ia-key"]
        )
        let plugin = InferenceAPIsPlugin()
        plugin.activate(host: host)
        ...
        _ = try await plugin.process(systemPrompt: "Fix grammar", userText: "hello world", model: nil)
